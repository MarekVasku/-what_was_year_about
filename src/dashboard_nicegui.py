...
```""")
    ...
    top3_lines = [
        "🥇 " + " • ".join([f"{row['Song']} ({row['Average Score']:.2f})" for _, row in top1.iterrows()])
        if not top1.empty
        else "🥇 —",
        "🥈 " + " • ".join([f"{row['Song']} ({row['Average Score']:.2f})" for _, row in top2.iterrows()])
        if not top2.empty
        else "🥈 —",
        "🥉 " + " • ".join([f"{row['Song']} ({row['Average Score']:.2f})" for _, row in top3.iterrows()])
        if not top3.empty
        else "🥉 —",
    ]
    top3_text = "\n".join(top3_lines)
