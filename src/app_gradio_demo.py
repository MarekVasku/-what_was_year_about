"""
Gradio DEMO/SHOWCASE web UI for the annual "What Was [Year] About" music chart.

This is identical to app_gradio.py except it pre-loads results on first visit.

Environment variables:
- DEMO_EMAIL_PREFIX: Email prefix to pre-load on first visit (required for demo mode)
...
Deploy to a separate HuggingFace Space for showcase purposes.
"""
...
# Get demo email from environment (fallback to empty string if not set)
DEMO_EMAIL_PREFIX = os.environ.get("DEMO_EMAIL_PREFIX", "")
...
with gr.Blocks(title="What was 2024 about chart", theme=theme, css=CUSTOM_CSS) as demo:
    ...
    with gr.Column(elem_classes=["hero"]):
        ...
        gr.HTML(
            ...
            "🎵 DEMO VERSION - Pre-loaded with sample results"
            "</p>"
            "<p style='color: white; font-size: 14px; margin: 0.5rem 0 0 0;'>"
            "You're seeing personalized insights as if someone already filled in their email. "
            "Enter your own email prefix below to see your results!"
            "</p>"
            "</div>"
        )
    ...
    # Initial load with demo email (pre-filled results, hidden from user)
    demo.load(
        lambda: (
